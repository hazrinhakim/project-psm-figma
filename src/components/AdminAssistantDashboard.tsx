...
interface AdminAssistantDashboardProps {
  user: User;
  onLogout: () => void;
}

type Module = 'dashboard' | 'assets' | 'qr' | 'maintenance' | 'feedback' | 'reports';

export function AdminAssistantDashboard({ user, onLogout }: AdminAssistantDashboardProps) {
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-3">
                ...
                />
                  <Badge 
                  variant="outline" 
                  className="rounded-lg bg-purple-100 text-purple-700 border-purple-600 text-[10px] py-0 px-1.5 h-5 font-medium">Admin Assistant</Badge>
              </div>
            </div>
            <Button
              variant="ghost"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="min-h-full rounded-2xl bg-white border border-slate-200 p-6 shadow-sm">
            ...
            {activeModule === 'dashboard' && <DashboardOverview role="admin_assistant" />}
