      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredAssets.map((asset) => (
            <div key={asset.id} className="bg-white rounded-xl p-5 shadow-sm border border-slate-200">
              ...
              <div className="space-y-2 mb-4">
                ...
                {asset.warrantyExpiry && (
                  <p className="text-xs text-slate-600">
                    🛡️ Warranty: {new Date(asset.warrantyExpiry).toLocaleDateString()}
                  </p>
                )}
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-xl max-w-2xl w-full p-6 my-8">
            <h3 className="text-slate-800 mb-4">{editingAsset ? 'Edit Asset' : 'Add New Asset'}</h3>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-slate-700 mb-2">Asset ID</label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    required
                    placeholder="e.g., Computer, Projector, Furniture"
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm text-slate-700 mb-2">Warranty Expiry (Optional)</label>
                  <input
                    type="date"
