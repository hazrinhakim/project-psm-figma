...
import { AssetView } from './modules/AssetView';
...
import { StaffDashboardOverview } from './modules/StaffDashboardOverview';
import {
...
type Module = 'dashboard' | 'assets' | 'qr' | 'maintenance' | 'feedback';
...
export function StaffDashboard({ user, onLogout }: StaffDashboardProps) {
  ...
  const markNotificationAsRead = async (notificationId: string) => {
    ...
    try {
      ...
      setNotifError('Failed to update notification status.');
    }
  };
  ...
  const menuItems = [
    ...
    { id: 'assets' as Module, label: 'View Assets', icon: Package },
    { id: 'qr' as Module, label: 'Scan QR Code', icon: QrCode },
    { id: 'maintenance' as Module, label: 'Maintenance Request', icon: Wrench },
    { id: 'feedback' as Module, label: 'Give Feedback', icon: MessageSquare }
  ];
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-3">
                ...
                />
                  <Badge 
                  variant="outline" 
                  className="rounded-lg bg-emerald-100 text-emerald-700 border-emerald-600 text-[10px] py-0 px-1.5 h-5 font-medium">Staff</Badge>
              </div>
            </div>
            <Button
              variant="ghost"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="min-h-full rounded-2xl bg-white border border-slate-200 p-6 shadow-sm">
            ...
            {activeModule === 'dashboard' && <StaffDashboardOverview userId={user.id} />}
            {activeModule === 'assets' && <AssetView user={user} />}
            {activeModule === 'qr' && <QRScanner />}
            {activeModule === 'maintenance' && <MaintenanceRequest user={user} />}
            {activeModule === 'feedback' && <FeedbackForm user={user} />}
          </div>
        </main>
      </div>
